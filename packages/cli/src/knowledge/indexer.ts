...
export class KnowledgeIndexer {
  ...
  async buildKB(
    ...
    try {
      const profilingModel = opts?.models?.profiling || opts?.models?.default || 'claude-sonnet-4-6';
    ...
    for (const repo of reposToIndex) {
      ...
      const totalChunkCount = Object.values(result.fileIndex).reduce((sum, f) => sum + f.chunkCount, 0);
    ...
    try {
      ...
      if (profiles.length > 0) {
        ...
        const meshModel = opts?.models?.service_mesh || opts?.models?.default || 'claude-sonnet-4-6';
    ...
    for (const repo of reposToIndex) {
      ...
      const totalChunkCount = result ? Object.values(result.fileIndex).reduce((sum, f) => sum + f.chunkCount, 0) : 0;
      if (sha) {
