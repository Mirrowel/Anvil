 ...
 */
export function createTaskBundlerStep(
  opts: TaskBundlerStepOptions = {},
): Step<string, TaskBundleOutput> {
  const id = opts.id ?? 'task-bundler';
  const artifactId = opts.repoName
    ? `${TASK_BUNDLES_ARTIFACT_ID}:${opts.repoName}`
  ...
  return {
    ...
    name: 'Task bundler',
    parallelism: 'serial',
    async run(ctx: StepContext<string>): Promise<TaskBundleOutput> {
      const tasksMd = typeof ctx.input === 'string' ? ctx.input : '';
      if (!tasksMd) {
        const empty: TaskBundleOutput = { tasks: [], groups: [] };
        opts.onBundle?.(empty);
        return empty;
      }
      ...
      const bundle: TaskBundleOutput = { tasks, groups };

      ctx.emit(artifactId, bundle);
