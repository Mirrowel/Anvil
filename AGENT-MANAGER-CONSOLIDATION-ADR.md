...
- **Deferred #2 — cli orchestrator broader migration — partially landed.**
  Survey found cli's main pipeline orchestrator (`cli/src/pipeline/orchestrator.ts`, 1682 LOC) shells out via `execSync` and does not consume either `AgentManager` class — there's nothing to migrate at the orchestrator layer. The actual cli sites that bare-spawn the agent binary are 4 commands: `diff` (wrapped Phase 5), `learn` (wrapped in this cleanup pass), `migrate`, `test-gen`. The latter two have non-trivial cache-key design (output depends on project source state, not just the prompt) — kept as future opt-in candidates rather than wrapped reflexively. cli's main orchestrator integration with `AgentManager` is gated on cli growing multi-agent state-tracking needs (currently it doesn't).
