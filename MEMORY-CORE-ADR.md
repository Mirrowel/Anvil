...
>
> **Status:** All 14 phases shipped 2026-04-29. See §8 for per-phase commit log.
...
| 14 — Migration importer + docs + ADR finalization | ✅ shipped 2026-04-29 | _this commit_ | New `memory-core/src/migrate/importer.ts`: `importLegacyMemories(legacyRoot, store, opts)` walks every `<root>/<dir>/memories.jsonl`, maps legacy `MemoryEntry` → v2 `Memory<string>` (`namespace = interpretLegacyDir(dir)`, legacy `kind` → v2 `subtype`, `provenance.createdBy='migration'` + `sourceRunId='pre-migration'`, `bitemporal.validAt = legacy.createdAt`, `decay.strength=100`), routes every payload through the Phase 7 scrubber (so credential-class secrets in legacy content hard-reject at import time, PII redacts in place). Idempotent: legacy `id` is preserved, so re-import is an upsert no-op. `dryRun` returns the plan without writes; `skipBackup` opts out of the per-file `.pre-migration.bak`. Plan §14 also re-titled the per-row entry to clarify §14 owns the importer + docs while §13 owns the inspector primitive (the original plan rows had them swapped after the dashboard pivot). README rewritten with public API quick-start, the five memory types, storage layout, bi-temporal semantics, sleeptime architecture diagram, env-var reference, migration guide. **Deviations:** §14.1 step 4 user-facing `anvil memory migrate` cli command deferred — `importLegacyMemories` is the API; cli wiring is the same one-liner that surfaces every other v2 inspector method (Phase 13 deferral) and lands in a follow-up that registers the cli/dashboard surfaces in one batch. §14.2 ADR "what shipped" finalization = this row's existence (every phase row now has a commit hash). 7 new migrate tests covering legacy → v2 mapping, namespace inference (project + global), .pre-migration.bak side effect, dryRun skip, idempotency, scrubber hard-reject of secrets in legacy data, missing-root no-op. memory-core 119/119 (8 legacy + 13 storage + 10 namespace + 7 bi-temporal + 9 drift + 15 scrubber + 10 retrieve + 7 ppr + 12 sleeptime + 8 reflect + 5 pr-episode + 8 inspector + 7 migrate); agent-core 81/81, knowledge-core 62/62, cli/mcp/dashboard builds clean. **Memory-core extract: complete.** |
